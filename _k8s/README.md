...
cluster sans CNI (CNI=none)  ─►  Cilium installé en Helm (README §9)
        ...
        ├─ 3. cert-manager/   TLS wildcard *.lab.ops.nc via Let's Encrypt DNS-01 Cloudflare
...
2. **Nom + TLS** — wildcard Cloudflare **public** `*.lab.ops.nc → 192.168.56.200`, en **DNS-only (nuage gris)** : le proxy Cloudflare ne peut pas joindre une IP privée `192.168.56.x`. Le TLS est donc terminé par **Envoy**, pas par Cloudflare → le Gateway doit porter un cert **publiquement trusté** (Let's Encrypt, cf. `cert-manager/`). Un cert *Cloudflare Origin CA* serait rejeté par les navigateurs.
