#!/usr/bin/env bash
#
# local-path-up.sh — installe Rancher local-path-provisioner (adapté Talos) : une
# StorageClass `local-path` PAR DÉFAUT qui provisionne des PV sur le disque local des
# workers (dossier /var/local-path-provisioner). Stockage NODE-LOCAL, sans réplication :
...
# ============================================================================
log "local-path-provisioner (adapté Talos : /var + namespace privileged)"
...
echo "  Chemin hôte  : /var/local-path-provisioner sur le worker qui héberge le PV"
