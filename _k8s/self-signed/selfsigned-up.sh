...
if [ -s "$CA_KEY" ] && [ -s "$CA_CRT" ]; then
  ...
  openssl req -x509 -newkey rsa:4096 -sha256 -nodes -days "$CA_DAYS" \
    ...
    -subj "/O=Vagrant-Talos lab/CN=Vagrant-Talos self-signed CA" \
...
EOF
  openssl req -new -newkey rsa:2048 -nodes \
    ...
    -subj "/O=Vagrant-Talos lab/CN=*.${LAB_DOMAIN}" 2>/dev/null
...
echo "    sudo cp ${CA_CRT#"$REPO_DIR"/} /usr/local/share/ca-certificates/vagrant-talos-lab.crt"
