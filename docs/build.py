...
latérale bascule tout le site et l'URL (`#fr/talos-upgrade`).
...
# `_k8s` est un SOUS-MODULE (dépôt k8s-playground, partagé avec le lab kubeadm) : sa
# documentation est construite et publiée par CE dépôt-là. L'inclure ici produirait deux
# copies divergentes de la même doc, et casserait le build en CI, où le sous-module n'est
# pas extrait (le dossier y est alors vide). Un lien externe le remplace dans le menu.
...
# ici en sous-module sur `_k8s/` et partagé avec le lab kubeadm : une seule source pour
# les deux, donc un seul endroit où la maintenir.
...
def lien_plateforme(mots: dict[str, str]) -> str:
    ...
    `_k8s/` est un sous-module pointant sur k8s-playground, partagé avec le lab kubeadm.
